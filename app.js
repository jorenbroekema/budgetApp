...
const budgetController = (function(){
    ...
    // IIFE return object, which includes access to variables above through closure
    return{
        addItem: function(type, des, val){ //add item return method
            let newItem, ID;
        ...
        deleteItem: function(type, id){
            let ids, index;
            ids = data.allItems[type].map(curr => {return curr.id});
        ...
        getPercentages: function(){
            return data.allItems.exp.map(cur => {return cur.getPercentage()});
        },
...
const UIController = (function(){
    ...
    // Store the class-names in an object for easy reference.
    const DOMstrings = {
        inputType: '.add__type',
    ...
    return{
        ...
        // Get input function to get the user filled in data on event triggers
        getInput: function(){
            return {
        ...
        // Add item to the UI list of expenses/incomes
        addListItem: function(obj, type){
            let html, newHtml, elem;

            if(type ==='inc'){
                html = '<div class="item clearfix" id="inc-%id%"><div class="item__description">%description%</div><div class="right clearfix"><div class="item__value">+ %value%</div><div class="item__delete"><button class="item__delete--btn"><i class="ion-ios-close-outline"></i></button></div></div></div>';
                elem = document.querySelector(DOMstrings.incomeContainer);
            }else if(type === 'exp'){
                html = '<div class="item clearfix" id="exp-%id%"><div class="item__description">%description%</div><div class="right clearfix"><div class="item__value">- %value%</div><div class="item__percentage">21%</div><div class="item__delete"><button class="item__delete--btn"><i class="ion-ios-close-outline"></i></button></div></div></div>';
                elem = document.querySelector(DOMstrings.expenseContainer);
            }

            newHtml = html.replace('%id%', obj.id);
            newHtml = newHtml.replace('%description%', obj.description);
            newHtml = newHtml.replace('%value%', obj.value);
            elem.insertAdjacentHTML('afterbegin', newHtml);
        ...
        clearFields: function(){
            ...
            fieldsArr = Array.prototype.slice.call(fields); // Method borrowing 'slice' to work on List object instead of Array
        ...
        // Function to access and store the DOMstrings object for reference by the global app controller
